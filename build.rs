        ...
        .map(|flag| {
            if target_env_is("msvc") {
                ...
                flag.strip_suffix(".lib").unwrap_or_else(|| {
                    panic!("system library '{}' does not appear to be a MSVC library file", flag)
                })
            } else {
                if let Some(flag) = flag.strip_prefix("-l") {
                    ...
                    if target_os_is("macos") {
                        ...
                        if let Some(flag) =
                            flag.strip_prefix("lib").and_then(|flag| flag.strip_suffix(".tbd"))
                        {
                            return flag;
                        }
                    }
...
fn get_link_libraries(
    ...
    let preferences = if *force { &one[..] } else { &both[..] }
        .iter()
        .map(|is_static| if *is_static { LibraryKind::Static } else { LibraryKind::Dynamic });
        ...
        .map(|name| {
            ...
            match kind {
                LibraryKind::Static => {
                    ...
                    if let Some(name) =
                        name.strip_prefix("lib").and_then(|name| name.strip_suffix(".a"))
                    {
                        // Unix (Linux/Mac)
                        // libLLVMfoo.a
                        name
                ...
                LibraryKind::Dynamic => {
                    ...
                    if let Some(name) =
                        name.strip_prefix("lib").and_then(|name| name.strip_suffix(".dylib"))
                    {
                        // Mac
                        // libLLVMfoo.dylib
                        name
                    } else if let Some(name) =
                        name.strip_prefix("lib").and_then(|name| name.strip_suffix(".so"))
                    {
                        // Linux
                        // libLLVMfoo.so
                        name
...
fn get_llvm_cflags(llvm_config_path: &Path) -> (String, Vec<String>) {
...
fn main() {
    ...
    if !cfg!(feature = "disable-alltargets-init") {
        let (all, includes) = get_llvm_cflags(&llvm_config_path);
